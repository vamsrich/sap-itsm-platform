...
router.post('/', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await auditLog({
      ...
      entityId: ci.id,
      newValues: { name: ci.name, ciType: ci.ciType, customerId },
    });
...
router.patch('/:id', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const old = await prisma.configurationItem.findFirst({
    ...
    const updated = await prisma.configurationItem.findUnique({
...
router.post('/seed', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const defaults = [
      ...
      {
        ciType: 'BTP_INSTANCE',
        name: 'SAP BTP Dev Subaccount',
        environment: 'DEV',
        sid: 'BTP-DEV',
        hostname: 'btp-dev.cfapps.eu10.hana.ondemand.com',
      },
      {
        ciType: 'BTP_INSTANCE',
        name: 'SAP BTP Production Subaccount',
        environment: 'PRD',
        sid: 'BTP-PRD',
        hostname: 'btp-prd.cfapps.eu10.hana.ondemand.com',
      },
      // Databases
      {
        ciType: 'DATABASE',
        name: 'HANA DB — ECC Production',
        environment: 'PRD',
        sid: 'HDB',
        hostname: 'hana-ecc-prd',
        version: 'HANA 2.0 SPS07',
      },
      {
        ciType: 'DATABASE',
        name: 'HANA DB — S/4HANA Production',
        environment: 'PRD',
        sid: 'HDB',
        hostname: 'hana-s4-prd',
        version: 'HANA 2.0 SPS07',
      },
      // Interfaces
      {
      ...
      },
      {
        ...
        environment: 'PRD',
        sid: 'CPI',
        hostname: 'cpi.it-cpi.cfapps.eu10.hana.ondemand.com',
      },
      // Fiori
      {
        ciType: 'SERVER',
