...
router.post('/templates', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!templateKey || !label || !subjectTemplate || !bodyTemplate) {
      res
        .status(400)
        .json({ success: false, error: 'templateKey, label, subjectTemplate, and bodyTemplate are required' });
      return;
    }
...
router.patch('/templates/:id', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      const sampleVars: Record<string, any> = {
        ...
        portalUrl:
          ...
          (process.env.CORS_ORIGINS ? process.env.CORS_ORIGINS.split(',')[0].trim() : 'https://app.example.com'),
      };
...
router.get('/metadata', enforceRole('SUPER_ADMIN', 'COMPANY_ADMIN'), async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (role === 'COMPANY_ADMIN') {
        ...
        if (!custId) {
          ...
          where.OR = [{ customerId: null }, ...(customer ? [{ customerId: customer.id }] : [])];
        } else {
          where.OR = [{ customerId: null }, { customerId: custId }];
        }
      } else if (filterCustomerId) {
        // Super admin filtering by customer
        where.OR = [{ customerId: null }, { customerId: filterCustomerId }];
      }
      ...
      const rules = await prisma.notificationRule.findMany({
        ...
        orderBy: [{ event: 'asc' }, { priority: 'asc' }, { customerId: 'asc' }],
      });
...
router.post('/preview', enforceRole('SUPER_ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { event, recordId, priority, customerId, toStatus } = req.body;
    const tenantId = req.user!.tenantId;

    // Find matching rules
    const rules = await findMatchingRules(tenantId, event, priority || null, customerId || null, toStatus || null);
