...
export function CMDBPage() {
  ...
  const columns: Column<any>[] = [
    { key: 'name', header: 'Name', render: (r) => <span className="font-medium text-gray-900">{r.name}</span> },
    {
      ...
      header: 'Type',
      render: (r) => (
        <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-medium">{r.ciType}</span>
      ),
    },
    {
      key: 'environment',
      header: 'Environment',
      render: (r) => <span className="text-sm text-gray-600">{r.environment || '—'}</span>,
    },
    {
      key: 'status',
      header: 'Status',
    ...
    },
    {
      key: 'customer',
      header: 'Customer',
      render: (r) => <span className="text-sm text-gray-600">{r.customer?.companyName || '—'}</span>,
    },
    {
      key: 'tickets',
      header: 'Open Tickets',
      render: (r) => <span className="text-sm font-semibold text-blue-600">{r._count?.records || 0}</span>,
    },
  ];
  return (
...
export function AuditPage() {
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      key: 'action',
      header: 'Action',
      render: (r) => <span className="text-xs font-bold px-2 py-0.5 rounded bg-blue-50 text-blue-700">{r.action}</span>,
    },
    {
      key: 'entity',
      header: 'Entity',
