...
export default function UsersPage() {
  ...
  const UserRow = ({ u, indent = false }: { u: any; indent?: boolean }) => {
    return (
      <div
        className={`flex items-center gap-3 py-3 px-4 ${indent ? 'ml-6 border-l-2 border-gray-100' : ''} hover:bg-gray-50 transition-colors rounded-lg group`}
      >
        {/* Avatar */}
        <div
        ...
        >
          {u.role === 'COMPANY_ADMIN' ? <Crown className="w-4 h-4" /> : <UserIcon className="w-4 h-4" />}
        </div>
        ...
        {u._count?.createdRecords > 0 && (
          <span className="text-xs font-medium text-orange-500 whitespace-nowrap">
            {u._count.createdRecords} ticket{u._count.createdRecords !== 1 ? 's' : ''}
          </span>
        )}
    ...
    <div className="p-6 space-y-5 max-w-screen-xl mx-auto">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
        ...
        <p className="text-xs text-blue-500 mt-0.5">
          Super Admin → Company Admin → Individual Users · Agents work across all companies
        </p>
      </div>
        ...
        <div className="space-y-4">
          {filteredGroups.map((group, idx) => {
            ...
            if (
              ...
              group.customer?.companyName?.toLowerCase().includes(search.toLowerCase())
            ) {
              return (
                <div
                ...
                >
                  ...
                  <div className="px-4 py-3 flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-gray-800">
                      {group.customer?.companyName || 'Unknown Company'}
                    </span>
                    <span className="text-xs text-gray-400 ml-auto">
                      {(group.admin ? 1 : 0) + group.users.length} user
                      {(group.admin ? 1 : 0) + group.users.length !== 1 ? 's' : ''}
                    </span>
                  </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={() => handleDeactivate(confirmDelete)} className="bg-red-600 hover:bg-red-700">
              Deactivate
            </Button>
          </>
        }
