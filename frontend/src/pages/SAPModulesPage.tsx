...
export default function SAPModulesPage() {
  ...
  const handleSaveModule = async () => {
    ...
    try {
      if (editModule) {
        await sapModulesApi.update(editModule.id, {
  ...
  const handleSaveSub = async () => {
    ...
    try {
      if (editSub) {
        await sapModulesApi.updateSubModule(editSub.id, {
          name: subForm.name,
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto space-y-5">
      <PageHeader
        title="SAP Modules"
        subtitle="Manage SAP functional modules and sub-modules for ticket categorization"
      />
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Click "Seed Default Modules" to create the standard SAP module hierarchy
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {allModules.map((mod) => {
            ...
            return (
              <div
                key={mod.id}
                className={`bg-white rounded-xl border border-gray-200 overflow-hidden ${!mod.isActive ? 'opacity-50' : ''}`}
              >
                {/* Module row */}
                <div className="flex items-center gap-3 px-5 py-3 group">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg ${mod.isActive ? 'text-green-500 hover:bg-green-50' : 'text-gray-400 hover:bg-gray-100'}`}
                    >
                ...
                {isExpanded && subModules.length > 0 && (
                  <div className="border-t border-gray-100 bg-gray-50/50">
                    {subModules.map((sub: any) => (
                      <div
                        key={sub.id}
                        className={`flex items-center gap-3 px-5 py-2.5 pl-14 group/sub border-b border-gray-100 last:border-0 ${!sub.isActive ? 'opacity-50' : ''}`}
                      >
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover/sub:opacity-100 transition-opacity">
                          <button
                            ...
                            className={`p-1 rounded-lg ${sub.isActive ? 'text-green-500' : 'text-gray-400'}`}
                          >
                            {sub.isActive ? (
                          ...
                          <button
                            onClick={() => openEditSub(sub, mod.id)}
                            className="p-1 text-orange-400 hover:bg-orange-50 rounded-lg"
                          >
                ...
                {isExpanded && subModules.length === 0 && (
                  <div className="border-t border-gray-100 px-14 py-6 text-center text-gray-400 text-sm">
                    No sub-modules yet.{' '}
                    <button onClick={() => openCreateSub(mod.id)} className="text-blue-600 hover:underline">
                      Add one
                    </button>
                  </div>
                )}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button loading={saving} onClick={handleSaveModule}>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button loading={saving} onClick={handleSaveSub}>
              {editSub ? 'Save' : 'Create'}
            </Button>
          </>
        }
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                value={subForm.code}
                onChange={(e) => setSubForm((f) => ({ ...f, code: e.target.value.toUpperCase() }))}
                disabled={!!editSub}
                placeholder="INV"
                maxLength={10}
                className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm font-mono focus:ring-2 focus:ring-blue-500 focus:outline-none disabled:bg-gray-100"
