...
const ic =
  'w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white';
...
function applyPreset(preset: (typeof PRESETS)[0]): SupportTypeForm {
  return {
    ...
    weekendCoverage: preset.weekendCoverage,
    holidayCoverage: preset.holidayCoverage,
    afterHoursCoverage: (preset as any).afterHoursCoverage || 'NONE',
    weekendMultiplier: 2.0,
    holidayMultiplier: 2.0,
    afterHoursMultiplier: 1.5,
...
export default function SupportTypeMasterPage() {
  ...
  const setF = <K extends keyof SupportTypeForm>(k: K, v: SupportTypeForm[K]) => setForm((f) => ({ ...f, [k]: v }));

  const toggleDay = (d: number) =>
    setF('workDays', form.workDays.includes(d) ? form.workDays.filter((x) => x !== d) : [...form.workDays, d].sort());

  const togglePause = (c: string) =>
    setF(
      'slaPauseConditions',
      form.slaPauseConditions.includes(c)
  ...
  const toggleOnCallPriority = (p: string) =>
    setF(
      'onCallPriorities',
      form.onCallPriorities.includes(p) ? form.onCallPriorities.filter((x) => x !== p) : [...form.onCallPriorities, p],
    );
  ...
  const handleCoverageChange = (field: 'weekendCoverage' | 'holidayCoverage' | 'afterHoursCoverage', val: string) => {
    setF(field, val);
  ...
  const handleOpen = (t?: any) => {
    if (t) {
      setForm({
        ...
        workDays: t.workDays || [1, 2, 3, 4, 5],
        dailyHours: t.dailyHours || 9,
        weekendCoverage: t.weekendCoverage || 'NONE',
        holidayCoverage: t.holidayCoverage || 'NONE',
        afterHoursCoverage: t.afterHoursCoverage || 'NONE',
        weekendMultiplier: t.weekendMultiplier || 2.0,
        holidayMultiplier: t.holidayMultiplier || 2.0,
        afterHoursMultiplier: t.afterHoursMultiplier || 1.5,
  ...
  const hasOnCall =
    form.weekendCoverage === 'ON_CALL' ||
    form.holidayCoverage === 'ON_CALL' ||
    form.afterHoursCoverage === 'ON_CALL' ||
    form.weekendCoverage === 'FULL' ||
    form.holidayCoverage === 'FULL' ||
    form.afterHoursCoverage === 'FULL';
    ...
    <div className="p-6 max-w-screen-xl mx-auto space-y-6">
      ...
      {isSuperAdmin && types.length === 0 && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
          <p className="text-sm font-semibold text-indigo-800 mb-3">Quick start — create from standard templates:</p>
          <div className="flex flex-wrap gap-2">
            {PRESETS.map((p) => (
              <button
                key={p.code}
                onClick={() => {
                  setForm(applyPreset(p));
                  setEditId(null);
                  setModal(true);
                }}
                ...
                <div className="p-5 flex flex-col gap-4 flex-1">
                      ...
                      {isSuperAdmin && (
                        <>
                          ...
                          <button
                            ...
                            title={
                              (t._count?.contracts || 0) > 0 ? `${t._count.contracts} contracts use this` : 'Delete'
                            }
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-gray-50 rounded-xl p-2.5">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${coverageBadge(t.weekendCoverage).badge}`}
                      >
                    ...
                    <div className="bg-gray-50 rounded-xl p-2.5">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${coverageBadge(t.holidayCoverage).badge}`}
                      >
                    ...
                    <div className="flex divide-x divide-gray-100">
                      {PRIORITIES.map((p) => {
                        ...
                        return (
                          <div
                            key={p}
                            className={`flex-1 flex flex-col items-center py-2 gap-1 ${enabled ? '' : 'opacity-40'}`}
                          >
                            <span
                              className={`text-xs font-bold ${enabled ? PRIORITY_COLORS[p].split(' ')[0] : 'text-gray-400'}`}
                            >
                              {p}
                            </span>
                            {enabled ? (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-2xl my-6 shadow-2xl flex flex-col">
            ...
            <div className="p-7 space-y-6 overflow-y-auto max-h-[75vh]">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ...
                  <span className="ml-2 text-xs text-gray-400 font-normal">
                    (select which days are regular business days)
                  </span>
                </label>
              ...
              {/* Weekend + Holiday + Post Business Hours Coverage */}
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Post Business Hours</label>
                  <div className="flex flex-col gap-2">
                    {COVERAGE_OPTIONS.map((opt) => (
                      <label
                        key={opt.value}
                        className={`flex items-center gap-2.5 p-2.5 rounded-xl border-2 cursor-pointer transition-all ${
                          form.afterHoursCoverage === opt.value
                            ? 'border-indigo-500 bg-indigo-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <input
                          type="radio"
                          name="afterHoursCoverage"
                          value={opt.value}
                          checked={form.afterHoursCoverage === opt.value}
                          onChange={() => handleCoverageChange('afterHoursCoverage', opt.value)}
                          className="accent-indigo-600"
                        />
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${opt.badge}`}>{opt.label}</span>
                      </label>
                  ...
                  <div className="flex gap-2">
                    {PRIORITIES.map((p) => (
                      <button
                        key={p}
                        type="button"
                        onClick={() => toggleOnCallPriority(p)}
                    ...
                    {PRIORITIES.map((p) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() =>
                            form.priorityScope !== 'P1_ONLY' || p === 'P1' ? toggleSLAEnabled(p) : undefined
                          }
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PAUSE_CONDITIONS.map((c) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={form.slaPauseConditions.includes(c.value)}
                        onChange={() => togglePause(c.value)}
