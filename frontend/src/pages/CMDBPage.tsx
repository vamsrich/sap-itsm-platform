...
const TYPE_COLORS: Record<string, string> = {
  ...
  BTP_INSTANCE: 'bg-cyan-100 text-cyan-700',
...
const ENV_COLORS: Record<string, string> = {
  ...
  QAS: 'bg-amber-50 text-amber-600',
  PRD: 'bg-red-50 text-red-600',
  SBX: 'bg-gray-50 text-gray-500',
  DR: 'bg-purple-50 text-purple-600',
...
export default function CMDBPage() {
  ...
  const ic =
    'w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none';
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Search name, SID, hostname…"
        ...
        <div className="ml-auto flex gap-2">
          <Button variant="secondary" onClick={handleSeed} loading={seeding}>
            <Zap className="w-4 h-4" /> Seed SAP Environments
          </Button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-medium">
            {allCIs.length === 0 ? 'No configuration items' : 'No items match filters'}
          </p>
        </div>
      ) : (
              ...
              {filtered.map((ci) => (
                <tr key={ci.id} className="hover:bg-gray-50 group">
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${TYPE_COLORS[ci.ciType] || 'bg-gray-100'}`}
                    >
                      {ci.ciType.replace('_', ' ')}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">
                    {ci.customer?.companyName || <span className="text-gray-300">Global</span>}
                  </td>
                  <td className="px-4 py-3">
                    {ci.environment && (
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded ${ENV_COLORS[ci.environment] || 'bg-gray-50'}`}
                      >
                        {ci.environment}
                      </span>
                    )}
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_COLORS[ci.status] || 'bg-gray-100'}`}
                    >
                      {ci.status}
                    </span>
                  </td>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={ic}
              placeholder="SAP ECC Production"
            />
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                value={form.sid}
                onChange={(e) => setF('sid', e.target.value.toUpperCase())}
                className={ic}
                placeholder="EP1"
                maxLength={10}
              />
          ...
          <div>
            ...
            <input
              value={form.hostname}
              onChange={(e) => setF('hostname', e.target.value)}
              className={ic}
              placeholder="sapecc-prd.company.local"
            />
          </div>
        </div>
      </Modal>
