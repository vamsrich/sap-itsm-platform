...
export default function ShiftsPage() {
  ...
  const toggleChannel = (ch: string) => setF('channels', { ...form.channels, [ch]: !form.channels[ch] });
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Define daily support time windows (start time, end time, timezone). Work days are configured in Support
            Types.
          </p>
        </div>
        {isSuperAdmin && (
      ...
      <div className="bg-indigo-50 border-l-4 border-indigo-500 rounded-lg px-4 py-3 text-sm text-indigo-800">
        ...
        Which <em>days</em> of the week support runs is defined by the Support Type. Multiple shifts can be assigned to
        a contract for global coverage.
      </div>
              ...
              shifts.map((s: any) => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1.5">
                        {[
                          ...
                          <span key={k} className={`text-xs ${ch[k] ? 'opacity-100' : 'opacity-20'}`}>
                            {icon}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${s.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                      >
                        {s.status}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg my-6 shadow-2xl">
            ...
            <div className="p-7 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setF('breakMinutes', parseInt(e.target.value) || 0)}
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    Billable: {calcBillable(form.startTime, form.endTime, form.breakMinutes)}
                  </p>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {[
                    ...
                    <label
                      key={key}
                      className={`flex items-center gap-2 border-2 rounded-xl px-4 py-2.5 cursor-pointer transition-all text-sm ${form.channels[key] ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500'}`}
                    >
                      <input
                        type="checkbox"
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 text-sm text-indigo-800">
                <strong>Note:</strong> Work days (Mon–Fri, Sun–Thu etc.) are configured in{' '}
                <strong>Support Type Master</strong>, not here. This shift defines only the daily time window.
              </div>
            </div>
