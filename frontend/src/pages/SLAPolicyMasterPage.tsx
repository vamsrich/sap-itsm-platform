...
const PRESETS = [
  {
    ...
    priorities: {
      ...
      P2: { response: 60, resolution: 480, enabled: true },
      P3: { response: 240, resolution: 1440, enabled: true },
      P4: { response: 480, resolution: 2880, enabled: false },
    },
  },
  {
    ...
    priorities: {
      ...
      P4: { response: 960, resolution: 5760, enabled: false },
    },
  },
  {
    ...
    priorities: {
      P1: { response: 60, resolution: 960, enabled: true },
      P2: { response: 240, resolution: 1920, enabled: true },
      P3: { response: 960, resolution: 5760, enabled: false },
      P4: { response: 1920, resolution: 11520, enabled: false },
    },
  },
  {
    name: 'P1 Only',
    ...
    priorities: {
      P1: { response: 15, resolution: 120, enabled: true },
      P2: { response: 60, resolution: 480, enabled: false },
      P3: { response: 240, resolution: 1440, enabled: false },
      P4: { response: 480, resolution: 2880, enabled: false },
    },
  },
];

const blankPriorities = {
  P1: { response: 15, resolution: 240, enabled: true },
  P2: { response: 60, resolution: 480, enabled: true },
  P3: { response: 240, resolution: 1440, enabled: true },
  P4: { response: 480, resolution: 2880, enabled: false },
...
export default function SLAPolicyMasterPage() {
  ...
  const setPriority = (p: string, field: string, val: any) =>
    setForm((f: any) => ({
      ...
      priorities: {
        ...f.priorities,
        [p]: { ...f.priorities[p], [field]: field === 'enabled' ? val : parseInt(val) || 0 },
      },
    }));
  ...
  const handleOpen = (p?: any) => {
    if (p) {
      setForm({
        ...
        warningThreshold: p.warningThreshold ?? 0.8,
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Define reusable response/resolution targets by priority. Assign to contracts.
          </p>
        </div>
        {isSuperAdmin && (
          <button
      ...
      <div className="bg-indigo-50 border-l-4 border-indigo-500 rounded-lg px-4 py-3 text-sm text-indigo-800">
        <strong>Design:</strong> Each policy (GOLD, SILVER, BRONZE, or custom) defines response and resolution targets
        per priority. If a customer negotiates different targets, create a new named policy — never modify an existing
        one that contracts depend on.
      </div>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">
                      Warning at {Math.round((p.warningThreshold || 0.8) * 100)}%
                    </span>
                    {isSuperAdmin && (
                      <>
                        <button
            ...
            <div className="p-7 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setF('warningThreshold', parseFloat(e.target.value))}
                    className={ic}
                  >
                    <option value={0.7}>70% elapsed</option>
                    <option value={0.75}>75% elapsed</option>
                    <option value={0.8}>80% elapsed</option>
                    <option value={0.85}>85% elapsed</option>
                    <option value={0.9}>90% elapsed</option>
                  </select>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="isActive" className="text-sm font-medium text-gray-700">
                  Active (available for contract assignment)
                </label>
              </div>
            </div>
