...
export default function ContractFormPage() {
  ...
  useEffect(() => {
    if (existingContract) {
      ...
      setForm({
        ...
        billingAmount: c.billingAmount ? String(c.billingAmount) : '',
  ...
  const handleSave = async () => {
    if (!isEdit && !form.contractNumber.trim()) {
    ...
    <div className="p-6 max-w-4xl mx-auto pb-28">
      <div className="flex items-center gap-4 mb-7">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{isEdit ? 'Edit Contract' : 'New Contract'}</h1>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7">
          ...
          <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {shifts
                    ...
                    .map((s: any) => (
                      <label
                        key={s.id}
                        className={`flex items-start gap-2 border-2 rounded-xl px-3 py-2.5 cursor-pointer transition-all text-sm ${form.shiftIds.includes(s.id) ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300 text-gray-600'}`}
                      >
                        <input
                          type="checkbox"
                          checked={form.shiftIds.includes(s.id)}
                          onChange={() => toggleId('shiftIds', s.id)}
            ...
            <F label="Support Type" hint="Defines work week, weekend/holiday coverage, and SLA pause conditions">
              <select
                value={form.supportTypeMasterId}
                onChange={(e) => setF('supportTypeMasterId', e.target.value)}
                className={ic}
              >
              ...
              {holidays.length === 0 ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  {holidays.map((h: any) => (
                    <label
                      key={h.id}
                      className={`flex items-start gap-2 border-2 rounded-xl px-3 py-2.5 cursor-pointer transition-all text-sm ${form.holidayCalendarIds.includes(h.id) ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 hover:border-gray-300 text-gray-600'}`}
                    >
                      <input
                        type="checkbox"
                        checked={form.holidayCalendarIds.includes(h.id)}
                        onChange={() => toggleId('holidayCalendarIds', h.id)}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7">
          ...
          <F label="SLA Policy" hint="Defines response/resolution targets and warning threshold per priority">
            <select
              value={form.slaPolicyMasterId}
              onChange={(e) => setF('slaPolicyMasterId', e.target.value)}
              className={ic}
            >
            ...
            {selectedSLA && (
              <div className="mt-3 border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="px-4 py-2 bg-gray-50 border-t border-gray-200 text-xs text-gray-500">
                  Warning sent at {Math.round((selectedSLA.warningThreshold || 0.8) * 100)}% elapsed
                </div>
              </div>
            )}
          </F>
          ...
          <div className="grid grid-cols-2 gap-5">
            <F label="Billing Amount">
              <input
                ...
                onChange={(e) => setF('billingAmount', e.target.value)}
                className={ic}
                placeholder="0"
              />
            </F>
            ...
            <F label="Billing Frequency">
              <select
                ...
                onChange={(e) => setF('billingFrequency', e.target.value)}
                className={ic}
              >
                {BILLING_FREQ.map((f) => (
              ...
              {form.autoRenewal && (
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <input
                    ...
                    onChange={(e) => setF('renewalNoticeDays', parseInt(e.target.value))}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-4 flex justify-between items-center z-30">
        <button
          onClick={() => navigate('/contracts')}
          className="px-5 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-600 hover:bg-gray-50 font-medium"
        >
          Cancel
        </button>
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-7 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-semibold disabled:opacity-60 transition-colors shadow-sm"
        >
