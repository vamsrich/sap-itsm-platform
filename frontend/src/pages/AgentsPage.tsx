...
export default function AgentsPage() {
  ...
  const handleFixRole = async (a: any) => {
    ...
    if (
      !window.confirm(
        `Fix role for ${a.user?.firstName} ${a.user?.lastName}?\nCurrent: ${a.user?.role} → Correct: ${correctRole}`,
      )
    )
      return;
    try {
  ...
  const handleDelete = async (a: any) => {
    if (
      !window.confirm(
        `Delete ${a.user?.firstName} ${a.user?.lastName}?\nThis removes both the agent record AND the user account permanently.`,
      )
    )
      return;
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      if (linkMode && !editId) {
        ...
        if (!linkEmail.trim()) {
          toast.error('Email required to link existing user');
          setSaving(false);
          return;
        }
        const res = await agentsApi.linkUser({
        ...
        toast.success(
          `${res.data.userName} linked as ${form.agentType === 'PROJECT_MANAGER' ? 'Project Manager' : 'Agent'} — role corrected to ${res.data.fixedRole}`,
        );
        qc.invalidateQueries({ queryKey: ['agents'] });
        setShowModal(false);
        setSaving(false);
              ...
              list.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${LEVEL_COLORS[a.level] || 'bg-gray-100 text-gray-600'}`}
                      >
                        {a.level}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full ${status?.color || 'bg-gray-100 text-gray-500'}`}
                      >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            title="Delete agent and user account"
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-indigo-800 to-violet-900 rounded-t-2xl flex-shrink-0">
              <h2 className="font-bold text-white text-lg">{editId ? 'Edit Team Member' : 'Add Team Member'}</h2>
              <button
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { val: 'AGENT', label: 'Agent', desc: 'Handles support tickets', icon: '🧑‍💼' },
                    {
                      ...
                      desc: 'Oversees customer accounts',
                      icon: '📋',
                    },
                  ].map((opt) => (
                    <button
                      key={opt.val}
                      type="button"
                      onClick={() => setF('agentType', opt.val)}
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-sm font-semibold ${form.agentType === opt.val ? 'text-blue-700' : 'text-gray-700'}`}
                        >
                          {opt.label}
                        </span>
              ...
              {!editId && linkMode && (
                <div>
                  ...
                  <p className="text-xs text-amber-700 mt-1.5 bg-amber-50 px-2 py-1 rounded-lg border border-amber-200">
                    ⚠ This will fix their user role to{' '}
                    <strong>{form.agentType === 'PROJECT_MANAGER' ? 'PROJECT_MANAGER' : 'AGENT'}</strong> and create an
                    agent record for them.
                  </p>
                </div>
              )}
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 flex-shrink-0">
              ...
              >
                {saving
                  ...
                  : editId
                    ? 'Save Changes'
                    : `Create ${form.agentType === 'PROJECT_MANAGER' ? 'PM' : 'Agent'}`}
              </button>
            </div>
          </div>
        </div>
